%% Behavioral Summary: Intonation Task
% Adapted to PRONET folder structure
% May 2025

%UPDATE including conditions
...
% read in subjects from subject list
for s = 1:length(stim_cond)
  fid = fopen([HPC_PATH,'/SCRIPTS/NEW_2024/','ses-',stim_cond{s}], 'r');
  S = textscan(fid, '%s');
  VP = S{1};
  fclose(fid);
end

% Initialize output structs before loop
INTONT = struct([]);
PHONEME = struct([]);

for subi = 1:length(VP)
    subj = VP{subi};
    subjID = subj(1:2);  
