function pronet_GLM_Speakerwise_Feb2026(VP)
    % PRONET_GLM_SPEAKERWISE_FEB2026 (MEGA-SESSION CONCAT)
    % 
    % 1. Creates Speaker-separated design matrices (m1 vs f2). 183
    % regressors!
    % 2. Fixes logfile Level errors using numeric Morph string mapping.
    % 3. CONCAT FIX: Automatically shifts onsets by (nV * TR) for subsequent runs.
    ...
    % Output Paths
    DESIGNDIR = fullfile(DATA_PATH, 'design_matrices_Speakerwise_concat');
    GLM_BASE  = fullfile(DATA_PATH, ['sub-' subj_id], 'rsa_firstlevel_Speakerwise_Feb2026');
    ...
    for t = 1:length(task_list)
        ...
        cum_time = 0; % to shift onset times of later sessions
        TR_sec = [];  
        ...
        for s = 1:length(stim_cond)
            ...
            % --- Regressors (with time shift)---
            for spkIdx = 1:2
                spk = speakers_to_split{spkIdx};
                for blk = 1:n_blocks
                    for lvl = 1:n_levels
                        idx = find(strcmp(T.Task, taskName) & strcmp(T.Speaker, spk) & ilevel_all == lvl & T.Block == blk);
                        reg_count = reg_count + 1;
                        names{reg_count} = sprintf('%s_%s_%s_lvl%d_blk%d', taskName, cond, spk, lvl, blk);
                        
                        if isempty(idx)
                            onsets{reg_count} = []; durations{reg_count} = [];
                        else
                            % convert to seconds + add cumulated time
        ...
        save(fullfile(task_design_dir, sprintf('design_%s_Speakerwise_concat.mat', taskName)), 'names', 'onsets', 'durations');
    end
    ...
    reg_file = fullfile(subj_folder, 'rp_18_Speakerwise_Feb2026.txt');
    dlmwrite(reg_file, mot18, 'delimiter', '\t', 'precision', '%.6f');

    % Loop for Task Estimation
    for t = 1:numel(task_list)
        task = task_list{t};
        dsgn = fullfile(DESIGNDIR, ['sub-' subj_id], sprintf('task-%s', task), sprintf('design_%s_Speakerwise_concat.mat', task));
    ...
    fprintf('GLM FEB2026 (MEGA-SESSION CONCAT) Complete: sub-%s\n', subj_id);
end