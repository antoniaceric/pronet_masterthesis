function pronet_fitting_phoneme_custom_ml()
global HPC_PATH DATA_PATH

% === OVERVIEW ===
% Fits psychometric data for the PHONEME task using a custom Maximum 
% Likelihood (ML) estimation for a Logistic function.
...
load(fullfile(DATA_PATH, 'psychometrics', 'summary_phoneme_task.mat'))
...
for ci = 1:n_conditions
    ...
    for i = 1:length(PHONEME_cond)
        ...
        disp([subject ' - ' cond ' - ' num2str((i/length(PHONEME_cond))*100, '%.1f') ' %'])
    end

    % Save separate voices
    fname_sep = fullfile(OUTDIR, ['phoneme_resp_fit_' cond '_separate.mat']);
    save(fname_sep, 'FitInfo_separate');

    % Save averaged voices
    fname_avg = fullfile(OUTDIR, ['phoneme_resp_fit_' cond '_average.mat']);
...
% Save all
fname_all = fullfile(OUTDIR, 'phoneme_resp_fit_all_conditions.mat');
save(fname_all, 'AllFitInfo');

%% === NEW SECTION: Generate and Save Fitted Curve Data Points ===
fprintf('\n--- Generating Fitted Curve Data Points (Custom ML - Phoneme) ---\n');
...
fname_fitted = fullfile(OUTDIR, 'phoneme_resp_fit_all_conditions_FITTED.mat');
